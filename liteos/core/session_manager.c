...
static mutex_t g_load_app_lock = PTHREAD_MUTEX_INITIALIZER;
#define MAX_REF_COUNT 255

/* record all service node and need mutex to avoid race */
struct list_head g_service_list;
static mutex_t g_service_list_lock = PTHREAD_MUTEX_INITIALIZER;
	...
	struct tc_ns_client_context *context)
{
	...
	context->params[3].memref.size_addr =
		(uint64_t)(uintptr_t)&dev_file->pkg_name_len;
	context->params[3].memref.buffer =
		(uint64_t)(uintptr_t)dev_file->pkg_name;
	/* Set public key len and public key */
	if (dev_file->pub_key_len) {
		context->params[2].memref.size_addr =
			(uint64_t)(uintptr_t)&dev_file->pub_key_len;
		context->params[2].memref.buffer =
			(uint64_t)(uintptr_t)dev_file->pub_key;
	} else {
	...
	const struct load_img_params *params, struct tc_ns_smc_cmd *smc_cmd)
{
	...
	mb_pack->operation.buffer_h_addr[2] = 0;
...
{
	if (!is_kernel_thread(OsCurrTaskGet())) {
...
static int load_image_by_frame(struct load_img_params *params,
	unsigned int load_times, unsigned int *ret_origin, enum secfile_type_t type)
{
	...
	unsigned int file_size, unsigned int *ret_origin, enum secfile_type_t type)
{
	...
	if (file_size % (params.mb_load_size - sizeof(int)))
		load_times += 1;
	ret = load_image_by_frame(&params, load_times, ret_origin, type);
free_mem:
	mailbox_free(params.mb_load_mem);
	mailbox_free(params.mb_pack);
...
{
	int ret;
	unsigned int ret_origin = TEEC_ORIGIN_COMMS;
	...
	if (ret == 1) { /* 1 means we need to load image */
		...
		ret = tc_ns_load_image(dev_file, context->file_buffer,
			context->file_size, &ret_origin, LOAD_TA);
		if (ret) {
			tloge("load image failed, ret=%x", ret);
			if (ret_origin != TEEC_ORIGIN_COMMS) {
				context->returns.origin = ret_origin;
				ret = EFAULT;
			}
			mutex_unlock(&g_load_app_lock);
			return ret;
...
static int close_session_thread_fn(uintptr_t arg, int len)
{
	struct tc_ns_dev_file *dev = (void *)arg;
	uint32_t index;
	struct tc_ns_service *service = NULL;
...
void close_unclosed_session_in_kthread(struct tc_ns_dev_file *dev)
{
	...
	char thread_name[OS_TCB_NAME_LEN] = { 0 };
	if (sprintf_s(thread_name, OS_TCB_NAME_LEN, "close_fn_%6d", dev->dev_file_id) < 0) {
		tloge("sprintf fail\n");
		return;
	}
	close_thread = kthread_run(close_session_thread_fn,
		dev, sizeof(uint64_t), thread_name);
