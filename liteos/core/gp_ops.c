...
static bool is_usr_refmem_valid(union tc_ns_client_param *client_param)
{
	...
	if (!access_ok(VERIFY_READ,
		(unsigned long)(uintptr_t)client_param->memref.buffer, size))
		return false;
...
{
	if ((!LOS_IsUserAddress((uintptr_t)addr) && LOS_PaddrQuery((void *)addr)))
		return true;
	...
	struct tc_op_params *op_params)
{
	...
	smc_cmd->ca_pid = OsCurrTaskGet()->taskID;
	smc_cmd->pid = OsCurrTaskGet()->processID;
	...
	struct tc_op_params *op_params)
{
	if (call_params->dev->pub_key_len == sizeof(uint32_t) &&
		...
		!is_kernel_thread(OsCurrTaskGet()) && (call_params->flags & TC_CALL_GLOBAL))
		return true;
