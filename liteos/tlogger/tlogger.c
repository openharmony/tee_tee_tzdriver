 ...
 */
#include "tlogger.h"
#include <linux/kernel.h> /* copy_to_user */
...
static struct tlogger_log *get_tlogger_log_by_minor(int minor)
{
	...
	list_for_each_entry(log, &m_log_list, logs) {
		if (log->misc_device.minor == MISC_DYNAMIC_MINOR)
			return log;
	}
...
static int process_tlogger_open(struct file *file)
{
	struct tlogger_log *log = NULL;
	struct tlogger_reader *reader = NULL;
	...
	log = get_tlogger_log_by_minor(MISC_DYNAMIC_MINOR);
	if (!log)
		return -ENODEV;
...
static int process_tlogger_release(struct file *file)
{
	struct tlogger_reader *reader = NULL;
	struct tlogger_log *log = NULL;
...
static ssize_t process_tlogger_poll(struct file *file,
	poll_table *wait)
{
	struct tlogger_reader *reader = NULL;
	struct tlogger_log *log = NULL;
...
{
	return (int)access_ok(VERIFY_READ, (unsigned long)(uintptr_t)arg, arg_len);
}
...
static int process_tlogger_ioctl(struct file *file,
...
#ifdef CONFIG_COMPAT
static int process_tlogger_compat_ioctl(struct file *file,
	int cmd, unsigned long arg)
{
	...
	uintptr_t addr, int size)
{
	...
	log->misc_device.name = TLOGCAT_DEV_NAME;
	log->misc_device.op = &g_logger_fops;
	...
	ret = misc_register_adapt(&log->misc_device);
	if (unlikely(ret)) {
...
static void unregister_tlogger(void)
{
	...
	list_for_each_entry_safe(current_log, next_log, &m_log_list, logs) {
		...
		misc_register_adapt(&current_log->misc_device);
